#!/usr/bin/env bash
# Plan A: using official USDT contract address
set -euo pipefail
...
### ===== DOCKER DAEMON (Desktop → Colima fallback) =====
...
### ===== CLEANUP PROBLEMATIC CONTAINERS =====
docker rm -f quantumai-usdt watchtower autoheal "$CTR" >/dev/null 2>&1 || true

### ===== .env TEMPLATE (REQUIRED VARS) =====
if [[ ! -f ".env" ]]; then
  cat > .env <<'ENVEXP'
# ZORUNLU: Aşağıyı DOLDURMADAN ÇALIŞMAZ
INFURA_PROJECT_ID=__PUT_YOUR_INFURA_PROJECT_ID__
ETH_SENDER_ADDRESS=0x0000000000000000000000000000000000000000
ETH_PRIVATE_KEY=__PUT_YOUR_PRIVATE_KEY_HEX__
GLI_DRY_RUN=1
ENVEXP
...
if [[ -z "$INFURA_PROJECT_ID" || "$INFURA_PROJECT_ID" == __PUT_YOUR_INFURA_PROJECT_ID__ ]] || \
   [[ -z "$ETH_SENDER_ADDRESS" || "$ETH_SENDER_ADDRESS" == 0x0000000000000000000000000000000000000000 ]] || \
   [[ -z "$ETH_PRIVATE_KEY" || "$ETH_PRIVATE_KEY" == __PUT_YOUR_PRIVATE_KEY_HEX__ ]]; then
  echo "[ERR] .env zorunlu alanlar boş. Doldur ve tekrar çalıştır."
  exit 1
fi

### ===== APP: app.py (Flask + /balance) =====
cat > app.py <<"PY"
