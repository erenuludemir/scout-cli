PROJECT_DIR="/Users/erenuludemir/QuantumAI-Dockerized-System.migrated.20250908_121149.migrated.20250908_121221 3"; mkdir -p "$PROJECT_DIR/ops" "$PROJECT_DIR/_backups" "$PROJECT_DIR/_logs/health"; cat > "$PROJECT_DIR/ops/qai_compose_hardening_autofix.sh" <<'BASH'
#!/usr/bin/env bash
set -Eeuo pipefail
...
BASH
chmod +x "$PROJECT_DIR/ops/qai_compose_hardening_autofix.sh"
cat > "$PROJECT_DIR/ops/qai_compose_hardening_apply.sh" <<'BASH'
#!/usr/bin/env bash
set -Eeuo pipefail
PROJECT_DIR="${PROJECT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
cd "$PROJECT_DIR"
...
BASH
chmod +x "$PROJECT_DIR/ops/qai_compose_hardening_apply.sh"
"$PROJECT_DIR/ops/qai_compose_hardening_apply.sh"