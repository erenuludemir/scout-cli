...
if [ -z "$COLIMA_SOCK" ] && [ -S "$HOME/.colima/default/docker.sock" ]; then
  COLIMA_SOCK="$HOME/.colima/default/docker.sock"
fi

if [ -z "$COLIMA_SOCK" ] && [ -S "$HOME/.colima/docker.sock" ]; then
  COLIMA_SOCK="$HOME/.colima/docker.sock"
fi

if [ -z "$COLIMA_SOCK" ] && command -v docker >/dev/null 2>&1; then
  docker context inspect colima > "$LOG_DIR/docker_context_colima.json" 2>/dev/null || true
  if [ -f "$LOG_DIR/docker_context_colima.json" ]; then
    COLIMA_SOCK="$(python3 - "$LOG_DIR/docker_context_colima.json" <<'PY'
import json,sys
p=sys.argv[1]
try:
